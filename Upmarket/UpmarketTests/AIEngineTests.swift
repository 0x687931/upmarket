...
final class AIEngineTests: XCTestCase {
    ...
    func testEngineAssetMappingIsDistinctAndCoversBothMaxAssets() {
        XCTAssertEqual(AIEngine.granite.asset, .upmarketAI)
        ...
        XCTAssertEqual(mapped, [.upmarketAI, .lfm25VL])
    }
    ...
    func testBothMaxAssetsAreReportedAndRequireMax() {
        // The download UI is data-driven over nativeModelStatuses; both AI assets must surface.
        let statuses = ModelManager.nativeModelStatuses(in: FileManager.default.temporaryDirectory)
        let keys = Set(statuses.map(\.key))
        XCTAssertTrue(keys.contains(ModelAsset.upmarketAI.rawValue))
