...
struct PythonWorker {
    ...
    nonisolated func analyse(fileURL: URL) -> ComplexityAdvice? {
    ...
    nonisolated func convert(fileURL: URL, title: String, useAI: Bool, password: String?) -> ConversionResult {
    ...
    nonisolated func checkModels() -> [ModelStatus] {
    ...
    nonisolated func setOfflineMode() {
        let manager = Python.import("upmarket_models.model_manager")
        manager.set_offline_mode()
    }

    nonisolated func downloadModel(key: String, progressFile: String) -> ModelDownloadResult {
        let manager = Python.import("upmarket_models.model_manager")
